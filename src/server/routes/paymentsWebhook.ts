...
// A payment that isn't one of our own invoice references - check if its
// amount matches one of the two admin-configured subscription plans
// (migration 0042) and, if so, auto-upgrade that payer's account tier.
// Paystack Payment Pages carry no reference we created ourselves, so amount
...
paymentsWebhookRouter.post("/", express.raw({ type: "application/json" }), async (req: Request, res: Response) => {
  ...
  if (!transaction) {
    // Not one of our own invoice payments (we never initialized a
    // reference for it) - check whether it's a subscription-tier upgrade
    // instead: an admin-pasted Paystack Payment Page link, matched back to
    // a tier by the exact amount it charges (see migration 0042).
    if (event === "charge.success") {
      await handleSubscriptionPayment(supabase, reference, data);
    }
    res.status(200).json({ received: true });
    return;
  }
