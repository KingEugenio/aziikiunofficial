...
export default function CustomerCRM({
  ...
  const AddCustomerFormContent = () => (
    <form onSubmit={handleSaveCustomer} className="space-y-3.5 text-xs leading-relaxed">
  ...
  const CustomerDetailsContent = ({ customer }: { customer: Customer }) => {
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start gap-4 border-b border-slate-100 pb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900 font-sans">{customer.name}</h3>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => triggerReminder(customer, metrics.balance)}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-sans">
          ...
          <div className="bg-slate-50 border border-slate-250/60 p-3 rounded-xl flex items-center gap-2.5">
            ...
            <div>
              ...
              <strong className="text-slate-800">{customer.email}</strong>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3 pt-2">
          ...
          <div className="space-y-2 max-h-[160px] overflow-y-auto pr-1">
            {transactions
              .filter(t => t.customerId === customer.id && t.businessId === currentBusiness.id)
              .map(t => (
            ...
            {transactions.filter(t => t.customerId === customer.id && t.businessId === currentBusiness.id).length === 0 && (
  ...
  return (
    <div id="customer-crm-root" className="grid grid-cols-1 lg:grid-cols-12 gap-6 text-slate-800">
      ...
      <div className="lg:col-span-4 bg-white border border-slate-200 rounded-2xl p-5 flex flex-col gap-4 shadow-sm shadow-emerald-500/5">
        ...
        {/* Add customer form - collapsible on mobile */}
        {isAddingCustomer && (
          <CollapsibleSection
            title={editingCustomerId ? "Edit Account Record" : "New Account Record"}
      ...
      <div className="lg:col-span-8">
        {selectedCustomer ? (
          <CollapsibleSection
            title={`${selectedCustomer.name} — ${selectedCustomer.category}`}
            icon={<User className="w-4 h-4" />}
            defaultOpen={!isMobile && !isTablet}
            className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm shadow-emerald-500/5"
          >
            <CustomerDetailsContent customer={selectedCustomer} />
          </CollapsibleSection>
        ) : (
