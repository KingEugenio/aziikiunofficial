...
import { GameController as Gamepad2, Target, TrendUp as TrendingUp, Certificate as Award, ArrowCounterClockwise as RotateCcw, PaperPlaneTilt as Send } from "@phosphor-icons/react";
import { api } from "../lib/api";

// Shapes returned by src/server/routes/gameSessions.ts / gameScores.ts
// (camelCase, not the raw snake_case table rows).
...
export function MoneyGame({ businessId, userId }: MoneyGameProps) {
  ...
  const loadSessions = async () => {
    try {
      setLoading(true);
      const response = await api.gameSessions.list(businessId);
  ...
  const startNewSession = async (scenario: Scenario) => {
    ...
    try {
      const session = await api.gameSessions.create({
        businessId,
        gameState: { scenario: scenario.id },
      });
      setCurrentSession(session);
    } catch (err) {
      console.error("Failed to start a new game session:", err);
    }
  };
  ...
  const submitAnswer = async (answer: string) => {
    ...
    // Computed here, not read back from state: setAnswers/setSessionScore
    // below don't take effect until the next render, so on the LAST
    // question saveSessionScore() (called in this same tick) would
    // otherwise save the score from before this answer was counted.
    ...
    if (currentQuestionIndex < selectedScenario.questions.length - 1) {
      ...
      // Session complete
      await saveSessionScore(finalScore, finalAnswers);
      setGameState("results");
    }
  };

  const saveSessionScore = async (finalScore: number, finalAnswers: Record<string, string>) => {
    if (!selectedScenario || !currentSession) return;

    try {
      await api.gameScores.create({
        businessId,
        sessionId: currentSession.id,
        scenarioId: selectedScenario.id,
        scenarioName: selectedScenario.name,
        score: finalScore,
        maxScore: selectedScenario.maxScore,
        performanceMetrics: {
          correctAnswers: Object.entries(finalAnswers).filter(
            ([qId, ans]) => ans === selectedScenario.questions.find((q) => q.id === qId)?.correctAnswer
          ).length,
          totalQuestions: selectedScenario.questions.length,
        },
      });
      await api.gameSessions.update(currentSession.id, {
      ...
      <div className="space-y-6 p-6">
        ...
        <div className="space-y-3">
          ...
          {SCENARIOS.map((scenario) => {
            const completed = scores.some((s) => s.scenarioId === scenario.id);
            const bestScore = scores.filter((s) => s.scenarioId === scenario.id).sort((a, b) => b.score - a.score)[0];
          ...
          <div className="space-y-4">
            ...
            {currentQuestion.type === "choice" ? (
              ...
              <div className="flex gap-2">
                ...
                />
                <button
                  onClick={() => submitAnswer(textAnswerRef.current?.value || answers[currentQuestion.id] || "")}
