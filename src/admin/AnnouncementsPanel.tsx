...
export default function AnnouncementsPanel() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    setIsSending(true);
    try {
      await api.admin.announcements.create(title, message);
      setTitle("");
      setMessage("");
      load();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={handleSend} className="border border-slate-200 rounded-2xl p-4 space-y-3">
        ...
        <p className="text-[11px] text-slate-500">Shown as a dismissible banner to every signed-in user, until they dismiss it.</p>
        <input
          type="text"
          required
          maxLength={200}
        ...
        >
          <Send className="w-3.5 h-3.5" /> {isSending ? "Sending..." : "Send to all users"}
        </button>
      </form>
