...
import { Eye, Download, User, Clock, Waveform as Activity, Pulse, Trash as Trash2, MagnifyingGlass } from "@phosphor-icons/react";
...
export function ActivityMonitoringPanel({ businessId }: { businessId?: string } = {}) {
  ...
  const loadActivityData = async () => {
    try {
      ...
      const [logsResponse, statsResponse] = await Promise.all([
        ...
        api.admin.activityLogs.stats(businessId),
      ]);
  ...
  const handleExport = () => {
    window.open(api.admin.activityLogs.exportUrl(businessId));
  };

  const handleViewUserSummary = async (userId: string) => {
    try {
      const response = await api.admin.activityLogs.userSummary(userId, businessId);
